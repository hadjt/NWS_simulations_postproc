        ...
        for yr in yrmat:
            ...
            for mi, mn in enumerate(mnmat):
                ...
                for grid_val in ["T", "U", "V"]:
                    ...
                    if (file_output_freq_ann == False) | (
                        file_output_freq_ann & (mi == 0)
                    ):
                        ...
                        lat_dim_nc = rootgrp_out.createDimension(lat_dim, 375)
                        lon_dim_nc = rootgrp_out.createDimension(lon_dim, 297)
                        time_counter_dim_nc = rootgrp_out.createDimension(
                            time_dim, None
                        )
                        axis_nbounds_dim_nc = rootgrp_out.createDimension(bnd_dim, 2)
...
def run_CEDA_ens_climatologies(
    skip_existing=False, Test=False, Testvar=None, file_output_freq_ann=True
):
    """
        ...
        for ens in HCCP_ens_mat_12:
            ...
            for date_lst, date_name in zip(
                ...
                for grid_val in ["T", "U", "V"]:
                    ...
                    for yi, tmp_date_lst in enumerate(
                        date_lst
                        ...
                        for td, tmp_date in enumerate(tmp_date_lst):  #
                            ...
                            if file_output_freq_ann:
                                ...
                                tmp_fname = (
                                    input_dir + file_pref + tmp_date[:4] + file_suff
                                )
                                mi = int(tmp_date[4:]) - 1
...
def main():
    ...
    run_CEDA_monthly(
        file_output_freq_ann=file_output_freq_ann
    )  # 2:15 ##17min PD, 9min/NWSPPE ens mem
