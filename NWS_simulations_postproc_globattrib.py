...
out_comment_EnsStat_diff_meth_1 = out_comment_EnsStat + ' To calculate the absolute difference between the two periods, we substract the future ensemble statistics from those of the present day.'

out_comment_EnsStat_diff_meth_2 = out_comment_EnsStat + ' To calculate the ensemble statistics relative to the present day baseline, for each ensemble member, we remove the future climatological mean from the present day climatological mean, and remove the future climatological standard deviation from the present day climatological standard deviation before calculating the ensemble statistics. This method is not available in the CEDA data set, but can be calculated by the expert user, who may find it useful for some applications.'
