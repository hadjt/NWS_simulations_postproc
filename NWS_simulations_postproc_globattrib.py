...
out_comment_EnsStat_diff = out_comment_EnsStat + 'To calculate the absolute difference between the two periods, we substract the future ensemble statistics from those of the present day - these are stored as ensmean, ensvar, ensstd and intvar. To calculate the uncertainty on the projected change, we also calculate the ensemble standard deviation statistics relative to the present day baseline: for each ensemble member, we remove the future climatological mean from the present day climatological mean before calculating standard deviation across the ensemble - this is stored as projstd.'
